...
| `LLAMA_ARG_SPEC_*` / `LLAMA_ARG_N_GPU_LAYERS_DRAFT` | llama.cpp speculative-decoding options forwarded to both local and `--ociman` GGUF servers, including draft model/repository selection, token and probability thresholds, draft KV-cache types, backend sampling, MoE placement, and draft GPU layers. Deprecated `LLAMA_ARG_DRAFT_MAX`/`MIN` aliases are intentionally not forwarded; use `LLAMA_ARG_SPEC_DRAFT_N_MAX`/`MIN`. For models with an embedded MTP predictor, `LLAMA_ARG_SPEC_TYPE=draft-mtp` can substantially improve generation throughput; support and useful settings are model-dependent. |
